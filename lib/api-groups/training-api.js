...
function initiateContentRecommendationGeneration (content) {
    var data = { };
...
TrainingAPI.prototype._wrapTaskExecution = function (taskInitiator, executionParams) {
    ...
    return function () {
        ...
        function initiationCallbackPoll (error, response) {
            ...
            if (!taskId) {
                callback(new Error('No taskId found in reponse!'));
                return;
            }
 ...
 * If the webhook options have been passed when initializing the
 * {@link IntellogoClient} object, a webhook will be used to signal task completion.
