 ...
 */
TrainingAPI.prototype.pollForTask = function (taskId, pollingIntervalMs) {
    var result = new events.EventEmitter();

    function pollTaskStatus () {
        var pollResponse = this.getTaskStatus(taskId);
        pollResponse.on('data', function (data) {
            if (data.status === 'finished') {
                ...
                setTimeout(pollTaskStatus.bind(this), pollingIntervalMs);
            }
        });
